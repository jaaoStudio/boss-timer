<template>
  <div class="bg-gray-800 rounded-lg shadow-md p-6 mb-6" v-if="selectedBoss">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <!-- 基本資訊 -->
      <div>
        <h3 class="text-lg font-medium text-gray-300 mb-2">{{ t('bossInfo.basicInfo') }}</h3>
        <div class="space-y-2">
          <div class="flex justify-between">
            <span class="text-gray-400">{{ t('bossInfo.respawnTime') }}</span>
            <span class="font-medium text-gray-200">{{ selectedBoss.min_respawn_minutes }} - {{ selectedBoss.max_respawn_minutes }} {{ t('bossInfo.minutes') }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-400">{{ t('bossInfo.description') }}</span>
            <span class="font-medium text-gray-200">{{ selectedBoss.description || t('bossInfo.none') }}</span>
          </div>
        </div>
      </div>
      ...
      <!-- 當前狀態 -->
      <div>
        <h3 class="text-lg font-medium text-gray-300 mb-2">{{ t('bossInfo.currentStatus') }}</h3>
        <div class="space-y-2">
          <div class="flex justify-between">
            <span class="text-gray-400">{{ t('bossInfo.channel') }}</span>
            <span class="font-medium text-gray-200">{{ selectedRecord?.channel }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-400">{{ t('bossInfo.status') }}</span>
            <StatusBadge :status="selectedRecord?.current_status" />
          </div>
          <div class="flex justify-between">
            <span class="text-gray-400">{{ t('bossInfo.recordTime') }}</span>
      ...
      <div v-if="selectedRecord?.respawn_min_time">
        <h3 class="text-lg font-medium text-gray-300 mb-2">{{ t('bossInfo.respawnCountdown') }}</h3>
        <div class="space-y-2">
          <div class="flex justify-between">
            <span class="text-gray-400">{{ t('bossInfo.earliestRespawn') }}</span>
            <CountdownTimer :target-time="selectedRecord.respawn_min_time" />
          </div>
          <div class="flex justify-between">
            <span class="text-gray-400">{{ t('bossInfo.latestRespawn') }}</span>
