<template>
  ...
  <div
    v-if="isEditMode"
    class="my-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-indigo-200 dark:border-indigo-700 bg-indigo-50 dark:bg-indigo-900/20 px-4 py-2.5"
  >
    ...
    <div class="flex items-center gap-2 shrink-0">
      <el-button size="small" @click="emit('reset')">{{ t('layout.reset') }}</el-button>
      <el-button size="small" type="primary" @click="emit('exit')">{{ t('layout.done') }}</el-button>
    </div>
  </div>
